...
public class MainCafe extends Application {
    ...
    private VBox createMenuBox(ListView<String> cartView) {
        ...
        CheckBox memberCheckBox = new CheckBox("Member");
    ...
    private void placeOrder(ListView<String> cartView, ListView<String> queueListView) {
        if (!currentOrder.isEmpty()) {
            try {
                ...
                CheckBox memberCheckBox = (CheckBox) menuBox.lookup(".check-box");
                boolean isMember = memberCheckBox != null && memberCheckBox.isSelected();

                Map<String, Integer> finalOrder = new HashMap<>();
                currentOrder.forEach((item, weightQuantity) -> {
                    int weight = itemWeights.getOrDefault(item, 1);
                    finalOrder.put(item, weightQuantity / weight);
                });

                cafeQueue.addOrder(new Order(orderId, finalOrder, isMember));
                queueList.clear();
